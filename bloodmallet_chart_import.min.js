...
function bloodmallet_chart_import() {
    ...
    for (var _i = 0; _i < chart_list.length; _i++) {
      ...
      if (_html_element) {
        ...
        if (state.chart_engine == "highcharts") {
          ...
          try {
            var tmp_styled_chart = styled_chart;
  ...
  function load_data(state) {
    ...
    request.open("GET", path_to_data + data_group + "/" + data_name + "?" + new Date().getTime(), !0);
  ...
  function update_chart(state, html_element, chart, count) {
    ...
    if (simulated_steps) {
      for (var simulation_step_position = 0; simulation_step_position < simulated_steps.length; simulation_step_position++) {
        var simulation_step = simulated_steps[simulation_step_position];
        var dps_array = [];

        for (var _i2 = 0; _i2 < dps_ordered_keys.length; _i2++) {
          simulation_step = simulated_steps[simulation_step_position];
          var tmp_simulation_steps = simulated_steps.slice();
          ...
          if (data_type === "azerite_traits_stacking" && dps_key_values[simulation_step] === undefined) {
            ...
            try {
              for (var _iterator = simulated_steps[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var simulation_step_clean = simulation_step;
  ...
  function get_category_name(state, key, data) {
    ...
    if (state.tooltip_engine == "wowdb") {
      ...
      _a.href = "http://www.wowdb.com/";

      try {
        _a.href += "items/" + data.item_ids[key];
      } catch (error) {
        if (debug) {
          console.log(error);
      ...
      if (_a.href.indexOf("items") > -1) {
        ...
        _a.href += "?itemLevel=" + _ilvl;

        if (data.hasOwnProperty("class_id") && data.hasOwnProperty("used_azerite_traits_per_item")) {
          _a.href += "&azerite=";
          _a.href += data.class_id + ":0";
      ...
      try {
        _a.href += "spells/" + data.spell_ids[key];
      } catch (error) {
        if (debug) {
          console.log(error);
      ...
      _a.dataset.tooltipHref = _a.href;

      try {
        _a.appendChild(document.createTextNode(data.languages[key][language_table[state.language]]));
      } catch (error) {
      ...
      styled_chart.tooltip.formatter = function () {
        ...
        for (var i = this.points.length - 1; i >= 0; i--) {
          ...
          if (this.points[i].y !== 0) {
            ...
            point_div.appendChild(document.createTextNode("\xA0\xA0" + Intl.NumberFormat().format(cumulative_amount)));
          }
        }
